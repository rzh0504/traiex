...
const bookmarksFontWeightSelect = document.getElementById("bookmarksFontWeight");
...
function handleCategoryTouchMove(e) {
  if (!touchDragState.isDragging || touchDragState.dragType !== 'category') return;
  e.preventDefault();
...
function setupCategoryEventListeners() {
  ...
  categoriesContainer.querySelectorAll('.category-name-input').forEach(input => {
...
function addBookmarkToCategory(catIndex) {
  const container = categoriesContainer.querySelector(`.category-bookmarks[data-cat-index="${catIndex}"]`);
  ...
  const newContainer = categoriesContainer.querySelector(`.category-bookmarks[data-cat-index="${catIndex}"]`);
...
function renderDockManager() {
  ...
  if (currentDockSites.length === 0) {
    activeDockContainer.innerHTML = '<div class="bookmarks-empty">暂无站点</div>';
  } else {
  ...
  const availablePresets = dockSitePresets.filter(preset => !currentUrls.includes(preset.url));
  
  if (availablePresets.length === 0) {
    presetDockContainer.innerHTML = '<div class="bookmarks-empty">所有预设已添加</div>';
  } else {
...
async function loadSettings() {
  try {
    ...
    bookmarkCategories = result.bookmarkCategories || JSON.parse(JSON.stringify(defaultBookmarkCategories));
    renderCategories();
    
    // Load dock sites
    currentDockSites = result.dockSites || JSON.parse(JSON.stringify(defaultDockSites));
    renderDockManager();
  } catch (error) {
...
async function importSettings(file) {
  try {
    ...
    if (settings.showDateTime !== undefined) showDateTimeCheckbox.checked = settings.showDateTime;
    if (settings.showDock !== undefined) showDockCheckbox.checked = settings.showDock;
    if (settings.showBookmarks !== undefined) showBookmarksCheckbox.checked = settings.showBookmarks;
    ...
    if (settings.searchBorderRadius !== undefined) updateBorderRadiusDisplay(settings.searchBorderRadius);
    if (settings.bookmarksFontWeight) bookmarksFontWeightSelect.value = settings.bookmarksFontWeight;
